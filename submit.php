...
/**
 * IMPORTANT: Never leak notices/warnings into the JSON response.
 * - Disable display_errors (log them instead)
 * - Use output buffering and flush it before sending JSON
 */
...
function json_response(int $status, array $data): void {
    // purge ANY previous output (warnings, echoes, BOM, etc.)
    while (ob_get_level()) ob_end_clean();
...
function data_url_to_png(string $dataUrl): string {
    if (!str_starts_with($dataUrl, 'data:image/')) throw new RuntimeException('Invalid selfie data URL.');
