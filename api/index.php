...
if ($method === 'POST' && $rel === '/submissions') {
  ...
  if (!is_array($inds) || count($inds) < 1)
    $err['industries'] = 'Select at least one industry';
  ...
  if (!is_array($apps) || count($apps) < 1)
    $err['applications'] = 'Select at least one application';
  $dataUrl = (string) ($s['data_url'] ?? '');
  if ($dataUrl === '')
    $err['selfie'] = 'Selfie is required';

  if (!empty($err))
    json_response(422, ['ok' => false, 'errors' => $err]);
